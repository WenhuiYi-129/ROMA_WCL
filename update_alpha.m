function [alpha] = update_alpha(p_max,noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_alpha, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, A)
...
alpha_opt = fmincon(@(alpha) objective(p_max,noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_alpha, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, rho), alpha, [], [], [], [], ...
                               lb, ub, [], opt);
alpha     = alpha_opt;
...
%======function======
% objective function
function cost = objective(p_max,noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_alpha, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, rho)
delta=0;
M=Mh*Mv;
N=Nh*Nv;
...
for n = 1:K+1
    cost = cost + rho * (alpha(n) - Z_alpha(n))^2;
end
