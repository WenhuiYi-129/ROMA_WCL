function [R_rr] = update_rr(noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_Rr, R_s, R_rr, u, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, A)
...
opt        = optimoptions('fmincon', 'MaxIterations', 500, 'Display','off');
% opt = optimoptions('fmincon', 'MaxIterations', 500);

R_opt = fmincon(@(R_rr) objective(noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_Rr, R_s, R_rr, u, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, rho), R_rr, [], [], [], [], ...
                               lb, ub, [], opt);
R_rr     = R_opt;
...
%======function======
% objective function
function cost = objective(noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_Rr, R_s, R_rr, u, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, rho)
delta=0;
M=Mh*Mv;
N=Nh*Nv;
