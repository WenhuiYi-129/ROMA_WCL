...
ENDpoint=zeros(length(Nter_list),1);
...
for iter = 1:Nter
    b=toc;
fprintf('now iter is %d/%d/200, pieces %d, times is %.4f\n', iter,Nter, cap_temp, b);
cap_before = cap_temp;


% update {r_s}, fix {r_r}, alpha, beta and {z_n}
[R_s] = update_rs(noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_S, R_s, R_r, theta_r, phi_r, theta_t,phi_t, alpha,beta,path_loss, lambda, A);
...
R_rr = R_r(:,:,u);
Z_Rr = Z_R(:,:,u);
R_rr = update_rr(noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_Rr, R_s, R_rr, u, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, A);
R_r(:,:,u) = R_rr;
[Z_Rr ] = update_z(Z_Rr , R_rr , d);
Z_R(:,:,u) = Z_Rr;
end

[alpha] = update_alpha(noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_alpha, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, A);
[Z_alpha ] = update_z(Z_alpha , alpha , d);

[beta] = update_beta(noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_alpha, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, A);
[Z_beta ] = update_z(Z_alpha , beta , d);
...
% hold on;
% xlabel('Normalized region size');
% ylabel('Average SE (bps/Hz)');
% grid on;
