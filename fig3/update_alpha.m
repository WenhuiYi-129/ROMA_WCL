function [alpha] = update_alpha(noise,location_u,Mh,Mv,Nh,Nv,K,L,R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, A)
...
alpha_opt = fmincon(@(alpha) objective(noise,location_u,Mh,Mv,Nh,Nv,K,L,R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, rho), alpha, [], [], [], [], ...
                               lb, ub, [], opt);
alpha     = alpha_opt;
...
%======function======
% objective function
function cost = objective(noise,location_u,Mh,Mv,Nh,Nv,K,L, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, rho)
delta=0;
M=Mh*Mv;
N=Nh*Nv;
