function [beta] = update_beta(p_max,noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_beta, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, A)
...
beta_opt = fmincon(@(beta) objective(p_max,noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_beta, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, rho), beta, [], [], [], [], ...
                               lb, ub, [], opt);
beta     = beta_opt;
...
%======function======
% objective function
function cost = objective(p_max,noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_beta, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, rho)
delta=0;
M=Mh*Mv;
N=Nh*Nv;
...
for n = 1:K+1
    cost = cost + rho * (beta(n) - Z_beta(n))^2;
end
