...
[alpha] = update_alpha(p_max, noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_alpha, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, A);
[Z_alpha ] = update_z(Z_alpha , alpha , d);

[beta] = update_beta(p_max, noise,location_u,Mh,Mv,Nh,Nv,K,L,Z_alpha, R_s, R_r, theta_r, phi_r, theta_t,phi_t,alpha,beta,path_loss, lambda, A);
...
hold on ;
h4=plot(Nter_list, ENDpoint(:,4));
hold on ;
h5=plot(Nter_list, ENDpoint(:,5));
hold on ;
...
% hold on;
legend('$p$=-10 dBm','$p$=0 dBm','$p$=10 dBm','$p$=20 dBm','Interpreter','latex' )
